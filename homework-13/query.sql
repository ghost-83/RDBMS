# Выборка всех добавленных композиций добавленных до 2023 года с группированных по жанру и выводом даты добавления 1 композиции и последней. Сортировка по жанру.
select m.genre, count(*), max(m.created) as last_entry_date, min(m.created) as date_of_first_entry
from ghost.music m
group by m.genre
having last_entry_date < '2023-01-01'
order by m.genre;

# Выборка с группировкой по имени пользователя и вывод количества добавленных композиций, а так же датой добавления первой и последней композиции. Вывод общего количества композиций и крайних дат через rollup.
select u.name,
       count(*),
